...
需要 Node.js 22 以上（Windows／macOS／Linux 都有現成的原生二進位檔，不必裝編譯工具）。
...
tools/                  開發用工具，不進映像
  generate-posters.js   產生 5–8 號的海報與橫式主視覺
  reframe-posters.js    把 1–4 號海報正規化成 2:3
...
| 方法 | 路徑 | 說明 |
|---|---|---|
| GET | `/api/config` | 提供給前端的公開設定（選位上限、票券時效、模擬登入開關） |
...
公開的展示連結最怕的是訪客點開之後看著空白頁等一分鐘，所以冷啟動的權重最高，
比區域延遲更值得優先考慮。兩個免費方案都不提供持久化磁碟，但對展示沒有影響——
...
換掉真實電影素材時需要四張新海報，但開發環境沒有任何影像處理套件
...
兩支工具都在 [`tools/`](tools) 底下，`npm run posters` 可以重新產生。
每部片的亂數由固定種子推導，重跑的輸出與版控裡的檔案位元完全相同。
`reframe-posters.js` 則刻意不是冪等的——邊緣的內陰影每跑一次就疊一層——
所以它會先檢查尺寸，已經是 800×1200 的就跳過。

會把產生器一起收進版控，是因為 README 上「海報是自製的」如果只是一句話，
那就只是宣稱；程式在旁邊，任何人都能重跑一次驗證。
這跟安全性表格每一列都對得回測試是同一件事。
...
- **登入／註冊有簡易速率限制**（每來源每分鐘：登入 15 次、註冊 30 次），
  計數存在行程記憶體內——多行程或多主機部署時要換成共用儲存才有效。
  另外，部署在反向代理後方（`TRUST_PROXY=1`）時，來源 IP 取自 X-Forwarded-For；
  若代理層是「附加」而不是「覆寫」這個標頭，攻擊者可以輪替偽造值繞過每 IP 上限。
  緩解方式是讓代理層覆寫 X-Forwarded-For，或改用共用儲存做計數。
- **票券「立即使用」不檢查場次日期**，任何時候都能把未使用的票標記為使用中；
  真正的驗票要在入場口比對場次時間。
- **登入成功後整頁重新整理**，訂票頁已選的座位會被清掉。
  這是刻意的取捨：側邊欄、頁尾都依登入狀態渲染，重載是最不容易漏掉狀態的做法。
...
- `pic/1`–`pic/4` 由 AI 生成，再以 [`tools/reframe-posters.js`](tools/reframe-posters.js)
  正規化為統一的 2:3 版面
- `pic/5`–`pic/8` 由 [`tools/generate-posters.js`](tools/generate-posters.js)
  以 HTML/SVG 排版後用 Chromium 截圖產生（見〈技術決策〉）
